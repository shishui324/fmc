...
//		circle_distence+=(Motor_control.Motor_Left_pid.present_value[0]+Motor_control.Motor_Right_pid.present_value[0])>>2;
		...
		if((Sensor.sum_16_34<80)&&(Sensor.sum_16_34>20))	//丢线判断
		{	
				Bell_Cry(100,100);
		...
		{
	//		//位置式
			Servo.output = (float)(Servo.kp *Servo.error[0] + Servo.kd* (Servo.error[0]-Servo.error[4]));     //5   2
...
//		circle_flag=true ;
    ...
    /*******************问题*****************
                差速是用算差还是算比
    ****************************************/
	Motor_control.Motor_Left_pid.present_value[0] = (int16)((float)(0.05*Motor_control.Motor_Left_pid.present_value[0])+(float)(0.95*getCountNum_L));
	Motor_control.Motor_Right_pid.present_value[0] = (int16)((float)(0.05*Motor_control.Motor_Right_pid.present_value[0])+(float)(0.95*getCountNum_R));
#if !(DEBUG_ON)
...
//	Motor.set_value[0] = 7;
	...
	Servo.output = 0.01*Speed.set_speed_val * Servo.output;  //0.05
	Motor_control.Motor_Left_pid.set_value[0] 	= Speed.set_speed_val - (int16_t)Servo.output;
	Motor_control.Motor_Right_pid.set_value[0] 	= Speed.set_speed_val + (int16_t)Servo.output;
