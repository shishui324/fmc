...
typedef struct
{
  uint16 zhidao_speed_val; //Ö±µÀËÙ¶È
  uint16 wandao_speed_val; //ÍäµÀËÙ¶È
  uint16 cross_speed_val;  //Ð¡SËÙ¶È
...
//	Key_Message_Init();
	...
	/*******参数初始化*******/
	  //速度
	Speed.cross_speed_val = 8;                 //20  6
	Speed.wandao_speed_val = 6;                //16  4
	Speed.zhidao_speed_val = 10;                //25  8
	
	Servo.kp = 4.20f;
    Servo.kd = 2.0f;   // 1.0  0.2
	...
	Motor_control.Motor_Left_pid.ki = 3.0;
	Motor_control.Motor_Left_pid.kp = 25.0;
	
	Motor_control.Motor_Right_pid.kvff = 1.0;
	Motor_control.Motor_Right_pid.kaff = 1.0;
	Motor_control.Motor_Right_pid.kd = 8.0;
	Motor_control.Motor_Right_pid.ki = 3.0;
	Motor_control.Motor_Right_pid.kp = 25.0;   //1 1 3 2 9
...
void servo_pid_caculate(void)           //差速控制pid
{
	...
	else
	{
		...
		isRightlose = false;
		/***********差方法************/
		if(Servo.output>SERVO_LIMIT_VAL||(sum_12<50.0&&sub_12>=0.0f))//Servo.output>=0))
		{
//			isLeftlose = true;
	        Servo.output = SERVO_LIMIT_VAL;
		}
...
void control(void)  //控制函数
{
    ...
    ****************************************/
	...
	speed_control();
//	Motor.set_value[0] = 4;
	Servo.output = 0.06 * Motor.set_value[0] * Servo.output;
...
//		Motor_control.Motor_Left_pid.set_value[0] = 0;
		Motor_control.Motor_Right_pid.set_value[0] = Motor_control.Motor_Right_pid.set_value[0]+10;
	}
	else if(isRightlose == true)
	{
		Motor_control.Motor_Left_pid.set_value[0] = Motor_control.Motor_Left_pid.set_value[0] + 10;
